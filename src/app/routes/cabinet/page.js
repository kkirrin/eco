    ...
    return(
        <>
          <main className={`${styles.main} ${styles.contentpage}`}>
             <h3 style = {{
               marginTop: "100px",
             }}>Авторизируйтесь, чтобы получить доступ к личному кабинету</h3>
          </main>
        </>
    )
  }
    ...
    <main className={`${styles.main} ${styles.contentpage}`}>
        ...
        <section className = {`${intostyles.mBlockCabinet}`}>
          ...
          <article className = {`${intostyles.lBlockCabinet}`}>
            ...
            <nav>
              <ul>
                <li onClick = {() => {setSelectPage(0)}}>Личные данные</li>
                <li onClick = {() => {setSelectPage(1)}}>Заказы</li>
                <li
                    onClick={() => exit(true)}
                    style = {{
                      color: "#626262",
        ...
        case 0:
            return(
                <div className = {intostyles.blockContent}>
                    <h2>Личные данные</h2>
                    <h5 style = {{margin: "10px 0 10px 0"}}>Статус клиента: {(customer.type) ? customer.type : ''}</h5>
    ...
    return(
        <>
         <div className={intostyles.rowButtons}>
            ...
            >Выполненные заказы</button>
         </div>
         <div className = {intostyles.listOfOrders}>
            {
                (customer && customer.orders && Array.isArray(customer.orders) && customer.orders[0] != 'undefined') ?
                        ...
                        if(item.OrderStatus != 'Выполнен' && typeOrders) {
                            return (
                                <div className={intostyles.oneOrder}>
                                    ...
                                    <p>
                                        Извещение не понадобится. <br/>А вот паспорт могут попросить, прежде чем вручить
                                        заказ
                                    </p>
                                    <div className={intostyles.infoDeliveryBlock}>
                                        ...
                                        <div className={intostyles.row}>
                                            ...
                                            <p>Получатель:</p>
                                            <p>{(item.Name && item.Phone) ? item.Name + "\n" + item.Phone : "Получатель неизвестен"}</p>
                                            <p>Статус:</p>
                                            <p>{(item.OrderStatus) ? item.OrderStatus : "Статус неизвестен"}</p>
                                            <p>Стоимость доставки:</p>
                                            <p>{(Number.parseInt(item.PriceDelivery)) ? item.PriceDelivery + " ₽" : "Стоимость неизвестна"}</p>
                                        </div>
                                    </div>
            ...
            return(
                <div className={intostyles.oneEditebleElement}>
                    ...
                    <div className={intostyles.downEditableElement}>
                        {
                            (editMode) ?
                                <input id="edit" name = {name} value = {value} onChange={(e) => {setValue(e.target.value)}} />
                                       :
                                <>
                                    {customer[check]}
                                </>
