        ...
        <section className = {stylesCart.mainCartBlock}>
            ...
            <article className = {stylesCart.mainRightBlock}>
                ...
                <div className = {`${stylesCart.totalSumBlock}`}>
                    ...
                    <button onClick = {(buttonText == "Оформить" && cart && cart[0]) ? getAllData : null} className = {`${stylesCart.buttonOrder}`}>{buttonText}</button>
    ...
    return(
        <article key = {`keyCartProduct_${product.id}`} className = {`${stylesCart.cartProduct}`}>
            ...
            <div className = {`${stylesCart.cartProductImage}`}>
                <Image unoptimized src = {`${process.env.NEXT_PUBLIC_PROTOCOL}://${process.env.NEXT_PUBLIC_URL_API}${(!Array.isArray(product.image)) ? product.image : product.image[0]}`} alt = {product.title} fill/>
            </div>
            <div className = {`${stylesCart.cartProductColumn}`}>
                <h3 className = {`${stylesCart.cartProductName}`}>{product.title}</h3>
                <p>Вес: {`${(product?.attributes) ? product.attributes.find(item => item.name == 'Вес')?.value : 0 }`} кг</p>
            ...
            <div className = {`${stylesCart.cartProductColumn}`}>
                ...
                <h3 className = {`${stylesCart.cartProductPrice}`}>{product.price * quantity} ₽</h3>
