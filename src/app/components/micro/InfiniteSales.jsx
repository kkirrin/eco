...
const InfiniteSales = ({speed = 0}) => {
    ...
    const {isLoading, error, data} = useGetSalesQuery()
    ...
    return(
        <div className = {`${styles.salesContainer}`}>
            ...
            {
                (!isLoading) ?
                    (typeof data != "undefined" && data.data && Array.isArray(data.data)) ? data.data.map((item, index) => {
                            return(
                                <div key = {`key_sales_${item.id}`} className={`${styles.oneSale}`}>
